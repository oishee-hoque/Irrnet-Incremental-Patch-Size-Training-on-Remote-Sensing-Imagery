...
class input_maker:
    ...
    def make_inputs(self):
        ...
        def Normalize_Rasters(imgIN):
            ...
            mean_rast = imgIN_.reduceNeighborhood(reducer=self.mean_reducer,
                                                           kernel=self.NORMALIZATION_KERNEL,
                                                               skipMasked = False).rename(bandNames)
            sd_rast = imgIN_.reduceNeighborhood(reducer=self.sd_reducer,
                                                           kernel=self.NORMALIZATION_KERNEL,
                                                               skipMasked = False).rename(bandNames)
        ...
        self.inputbands = ee.Image.cat([self.imgBands,season_stdDev])
        ...
        while not GOTBANDS:
            if cnt > 3:
                break
            try:
                self.BANDNAMES = self.inputbands.bandNames().getInfo()
                print(self.BANDNAMES)
                GOTBANDS = True
            except:
                print('Could not get the band names, trying again on')
            cnt += 1
